...
def criar_janela_importar_resultados(evento_id: int, evento_nome: str) -> sg.Window:
    ...
    Baseada no arquivo casos-de-uso/importaResulta.py.
...
def processar_importacao(janela: sg.Window, evento_id: int, arquivo_csv: str) -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        if erros:
            ...
            # Mostrar detalhes dos erros em popup scrollable (formato 16:9)
...
def _extrair_id_evento(evento_string: str) -> Optional[int]:
    """
    Extrai o ID do evento da string selecionada no ComboBox.
    
    Args:
        evento_string: String do evento (ex: "Meia Maratona - 10/08/2025 (21km)")
        
    Returns:
        ID do evento ou None se não encontrado
    """
    eventos_disponiveis = obter_eventos_para_importacao()
    
    for evento in eventos_disponiveis:
        item = f"{evento.nome} - {evento.data} ({evento.distancia})"
        if item == evento_string:
            return evento.id
...
def executar_janela_importacao(evento_id: int, evento_nome: str) -> bool:
    """
    ...
    while True:
        ...
        if event == '-IMPORTAR-':
            ...
            sucesso_importacao, mensagem = processar_importacao(janela, evento_id, arquivo)
...
if __name__ == "__main__":
    # Teste da janela de importação
    print("=== Teste da Janela de Importação ===")
    ...
    if sucesso:
        ...
        print("Importação cancelada ou com erro.")
